...
for line in file:
  ...
  # For each of my favourites see if it's in theirs too
    for fave in my_favorites[2:8]:
        if fave in their_favourites[2:8]:
...
#if their similarity scire is high enough (>3) , add to top friend
    if common_interest_counter >3:
        top_friends += [their_name]
